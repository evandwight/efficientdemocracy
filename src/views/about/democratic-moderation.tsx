...
const element = () =>
    <div>
        <h2>{title}</h2>
        <p>
            We want to make decisions by estimating a referendum. We have the following tools:
        </p>
        <ul>
            <li>Unsolicited votes - 0 cost* and timely but inaccurate due to selection bias</li>
            <li><a href={C.URLS.ABOUT_ELECTED_MODERATORS}>Elected representatives</a> - 0 cost and timely but inaccurate due to only indirect influence over the decisions</li>
            <li>Statistical sample - mostly accurate but constant cost and not timely</li>
            <li>Referendum - accurate but N cost and not timely</li>
        </ul>
        <p>
            * cost in terms of uninterested user time per decision
        </p>
        <p>
            Though each tool is fatally flawed, combining them allows us to overcome that. The efficiency of elected moderators lets them act as a first line of defense. They make the majority of decisions and prevent a flood of requests on the community's time. We detect bad decisions with unsolicited votes - disputes - and then correct the decision by triggering a statistical sample. Similarly, statistical samples can be corrected by a referendum. The efficient corrected by the accurate.
        </p>
        <p>
            Disputes have a selection bias so they can't be trusted to represent the will of the people. As such, we disincentivize abuse by empowering the community to give strikes to disputers when they feel the issue should not have been disputed.
        </p>
        <p>
            This system hybridizes a representative democracy with a direct democracy to share benefits and minimize the flaws.
        </p>
